        ...
        if offer_type == 1 {
            for i in 0..self.offers_sell.len() {
                if self.offers_sell.get(i).unwrap().offer_id == offer_id {
                    env::log(format!("remainig amount en float: {} - remainig amount en yocto as u128: {} - atached deposit as f64: {}", self.offers_sell[i].remaining_amount ,
                    (self.offers_sell[i].remaining_amount * YOCTO_NEAR as f64) as u128, attached_deposit as f64).as_bytes());
